...
void WindowUtils::MakeWindowForeground(HWND wnd) {
  ...
  if (!SetWindowPos(wnd,
	...
	_tprintf(_T("[WindowUtils::MakeWindowForeground] [SetWindowPos failed][0x%08x]"), HRESULTFromLastError());
...
HRESULT WindowUtils::SetWindowIcon(HWND hwnd, WORD icon_id, HICON* hicon) {
  ...
  *hicon = NULL;
  ...
  if (!icon) {
	...
	_tprintf(_T("[SetWindowIcon - LoadImage failed][0x%x]"), hr);
    return hr;
  }
